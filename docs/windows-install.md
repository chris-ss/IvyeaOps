...
- 如果下载的是 `IvyeaOps-Windows-x64.zip`，后端已打成 `IvyeaOpsServer.exe`，不需要安装 Python；双击「安装 IvyeaOps Windows x64.bat」即可。安装器会自动生成登录信息文件并直接启动浏览器。若直接双击 `IvyeaOpsServer.exe`，它也会自动生成桌面快捷方式和登录信息文件。
