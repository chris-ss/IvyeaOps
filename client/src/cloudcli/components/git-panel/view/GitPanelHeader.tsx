...
export default function GitPanelHeader({
  ...
  const request发布Confirmation = () => {
    onRequestConfirmation({
      type: 'publish',
      message: `发布 branch "${currentBranch}" to ${remoteName}?`,
      onConfirm: onPublish,
    });
  };
            ...
            <>
              {!remoteStatus.hasUpstream ? (
                <button
                  ...
                  title={`发布 "${currentBranch}" to ${remoteName}`}
                >
                  <Upload className={`h-3 w-3 ${isPublishing ? 'animate-pulse' : ''}`} />
                  {!isMobile && <span>{isPublishing ? '发布中…' : '发布'}</span>}
                </button>
              ) : (
                <>
                  {/* 获取 — always visible when remote exists */}
                  <button
                    ...
                    title={`获取 from ${remoteName}`}
                  >
                    <RefreshCw className={`h-3 w-3 ${isFetching ? 'animate-spin' : ''}`} />
                    {!isMobile && <span>{isFetching ? '拉取中…' : '获取'}</span>}
                  </button>
                  ...
                  {behindCount > 0 && (
                    ...
                    >
                      ...
                      {!isMobile && <span>{isPulling ? '拉取中…' : `Pull ${behindCount}`}</span>}
                    </button>
                  )}
                  ...
                  {aheadCount > 0 && (
                    ...
                    >
                      ...
                      {!isMobile && <span>{isPushing ? '推送中…' : `Push ${aheadCount}`}</span>}
                    </button>
                  )}
                </>
              )}
