...
export const getAllSessions = (project: Project): SessionWithProvider[] => {
  const mainSessions = [...(project.sessions || [])].map((session) => {
    // The backend provides session.agent for the actual agent id
    // (e.g. 'claude', 'codex', 'hermes', 'agy').
    const providerStr = (session.agent || 'claude') as LLMProvider;
    return {
      ...session,
      __provider: providerStr,
    };
  });
  ...
  const hermesSessions = (project.hermesSessions || []).map((session) => ({
    ...session,
    __provider: 'hermes' as const,
  }));

  const agySessions = (project.agySessions || []).map((session) => ({
    ...session,
