...
export default function ChangesView({
  ...
  const requestFileAction = useCallback(
    (filePath: string, status: FileStatusCode) => {
      ...
      onRequestConfirmation({
        type: 'discard',
        message: `丢弃 "${filePath}" 的所有更改？此操作无法撤销。`,
        onConfirm: async () => {
      ...
      <div className="flex-1 overflow-y-auto">
        {isLoading ? (
          ...
          <div className={isMobile ? 'pb-4' : ''}>
            ...
            {selectedFiles.size === 0 ? (
              <div className="px-3 py-2 text-xs text-muted-foreground italic">暂无已暂存文件</div>
            ) : (
              <FileChangeList
                gitStatus={gitStatus}
                gitDiff={gitDiff}
            ...
            {unstagedFiles.size === 0 ? (
              <div className="px-3 py-2 text-xs text-muted-foreground italic">所有变更已暂存</div>
            ) : (
              <FileChangeList
                gitStatus={gitStatus}
                gitDiff={gitDiff}
