...
- **可选组件**：Hermes Agent、GBrain、Ollama、Claude Code、Codex 不强塞主包，可在安装脚本、首启向导或系统配置里一键安装 / 修复。
...
- **高自由度 · 可定制**：板块与功能都能按需增删改；对现成功能不满意，可直接让内置智能体（Claude / Codex / Hermes）读代码帮你修改调优 —— 审核制修复还会在 git worktree 隔离、人工确认后才落地。
- **一次登录，全部模块**：单点登录后侧边栏直达所有板块，统一主题（内置 10 套配色）。
- **智能体驱动**：浏览器内直接跑 Hermes / Claude / Codex 智能体，支持流式输出、会话续聊、工具可视化。
...
<table>
  ...
  <tr>
    <td align="center" colspan="2">
      <img src="docs/assets/screenshot-gbrain.png" alt="GBrain 知识库：本地检索 + Hermes 对话，不开公网端口" width="50%" />
      <br />
      <sub><strong>GBrain 知识库</strong>：先检索本地知识库、再调用本机 Hermes 生成回答并标注引用来源，全程不新增公网端口</sub>
    </td>
  </tr>
</table>
...
> - **Hermes / GBrain / Ollama 等可选组件慢**：这些组件会访问境外安装器、npm/pip 或模型源，可选、可稍后单独重试；镜像环境变量会顺带加速其中的 pip/npm 步骤。
...
IvyeaOps 支持在浏览器里直接调用本地智能体 CLI；这些都是**可选组件**，没装也可以用「全局兜底大模型」跑大部分 AI 功能。首启向导和「系统配置 → 系统状态」都提供安装 / 修复入口：

| 智能体 / 组件 | 说明 |
|---|---|
| **Hermes Agent** | 推荐主力智能体，支持 MCP / 工具调用 / Skill；可一键安装或稍后修复 |
| **GBrain** | 本地知识库 CLI，初始化 `~/brain`，用于私有笔记与检索 |
| **Ollama** | 本地模型运行环境；用于 GBrain embedding 或本地大模型工作流，可一键安装或检测 |
