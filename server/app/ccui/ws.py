"""WebSocket endpoints for the CloudCLI backend: chat (``/ws``) and shell
(``/shell``).
...
from app.ccui import claude_driver, codex_driver, hermes_driver
...
async def shell_ws(websocket: WebSocket) -> None:
    if not _authed(websocket):
        ...
        return
    await websocket.accept()
    from app.ccui.shell_pty import ShellConnection
