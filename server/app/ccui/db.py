"""SQLite layer for the native CloudCLI backend.

We reuse the same database file the old Node service wrote to
(``~/.cloudcli/auth.db`` by default, overridable via ``CCUI_DB_PATH``) so all
...
def get_db_path() -> Path:
    ...
    carries over; override with ``CCUI_DB_PATH`` for tests/fresh installs."""
    override = os.getenv("CCUI_DB_PATH")
    if override:
        return Path(override)
    return Path.home() / ".cloudcli" / "auth.db"
