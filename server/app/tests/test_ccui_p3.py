...
def ctx(tmp_path: Path, monkeypatch):
    ...
    from app.ccui.routers import files as files_mod
...
def test_file_tree(ctx):
    c, _proj, _ = ctx
    r = c.get("/api/ccui/projects/p1/files")
...
def test_read_and_save(ctx):
    c, *_ = ctx
    r = c.get("/api/ccui/projects/p1/file", params={"filePath": "a.txt"})
    assert r.status_code == 200 and r.json()["content"] == "hello"
    r = c.put("/api/ccui/projects/p1/file", json={"filePath": "a.txt", "content": "world"}, headers=_HDR)
    assert r.status_code == 200 and r.json()["success"]
    assert c.get("/api/ccui/projects/p1/file", params={"filePath": "a.txt"}).json()["content"] == "world"


def test_create_rename_delete(ctx):
    c, *_ = ctx
    assert c.post("/api/ccui/projects/p1/files/create", json={"path": "", "type": "file", "name": "b.txt"}, headers=_HDR).json()["success"]
    assert c.put("/api/ccui/projects/p1/files/rename", json={"oldPath": "b.txt", "newName": "c.txt"}, headers=_HDR).json()["newName"] == "c.txt"
    assert c.request("DELETE", "/api/ccui/projects/p1/files", json={"path": "c.txt", "type": "file"}, headers=_HDR).json()["success"]


def test_binary_content(ctx):
    c, *_ = ctx
    r = c.get("/api/ccui/projects/p1/files/content", params={"path": "a.txt"})
    assert r.status_code == 200 and r.content == b"hello"


def test_path_traversal_blocked(ctx):
    c, *_ = ctx
    r = c.get("/api/ccui/projects/p1/file", params={"filePath": "../../../etc/passwd"})
    assert r.status_code == 403


def test_upload_files(ctx):
    c, proj, _ = ctx
    r = c.post("/api/ccui/projects/p1/files/upload",
...
def test_upload_images(ctx):
    c, *_ = ctx
    r = c.post("/api/ccui/projects/p1/upload-images",
...
def test_browse_filesystem(ctx):
    c, _proj, root = ctx
    r = c.get("/api/ccui/browse-filesystem", params={"path": root})
    assert r.status_code == 200, r.text
...
def test_create_folder(ctx):
    ...
    r = c.post("/api/ccui/create-folder", json={"path": target}, headers=_HDR)
    assert r.status_code == 200 and r.json()["success"]
